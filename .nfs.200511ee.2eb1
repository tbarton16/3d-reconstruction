...
%     add ones
    ...
    [U S V] = svd(P);
    camMat = V(:, end);
    camMat = reshape(camMat, 4,3);
%   unnormalize the points here
    camMat = ((camMat/sqrt(2)) * all_sd) + all_mean;
    test_pts =  camMat* proj_pts(1,:)' 
    test_pts =  camMat* proj_pts(2,:)'
    test_pts =  camMat* proj_pts(3,:)'
    test_pts =  camMat* proj_pts(4,:)'
    test_pts =  camMat* proj_pts(5,:)'
    test_pts =  camMat* proj_pts(6,:)'
    
    real_pts = ((real_pts/sqrt(2)) * all_sd) + all_mean;
    disp(real_pts)
end
